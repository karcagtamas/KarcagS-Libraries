...
public class AutoListTableDataSource<T, TKey> : DataSource<T, TKey> where T : class, IIdentified<TKey>
{
    ...
    private AutoListTableDataSource(Func<QueryModel, Task<IQueryable<T>>> fetcher)
    {
        Fetcher = fetcher;
    }

    public static AutoListTableDataSource<T, TKey> Build(Func<QueryModel, Task<IQueryable<T>>> fetcher) => new(fetcher);

    public AutoListTableDataSource<T, TKey> SetTextFilteredColumns(params string[] keys)
    {
    ...
    public AutoListTableDataSource<T, TKey> SetEFFilteredEntries(params string[] names)
    {
    ...
    public override async Task<IEnumerable<T>> LoadDataAsync(QueryModel query, Configuration<T, TKey> configuration)
    {
        ...
        fetcherQuery = fetcherQuery.ApplyPagination(query.Page, query.Size);
    ...
    private static IQueryable<T> ApplyTextFilter(Column<T, TKey> column, IQueryable<T> query, string filter) =>
        query.Where(obj => ((string?)column.ValueGetter(obj).GetAwaiter().GetResult() ?? "").ToLower().Contains(filter.ToLower()));
