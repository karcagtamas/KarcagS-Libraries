...
public class MaxSumAttribute : ValidationAttribute
{
    ...
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
        ...
        return val + other <= MaxSum ? ValidationResult.Success : new ValidationResult(GetMessage(MaxSum));
    }

    protected virtual string GetMessage(int sum) => $"{Message} {sum}";
}